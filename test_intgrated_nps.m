...
for j=1:k 
    for i=1:n
        ...
        d = length(nonzero_indices) + 1;
        min_nps_zero = nps(d);
        min_nps_nonzero = nps(d-1);
        f_max = f(d);
        noise_test(j,i) = Q_30(j,i) * (3.14 * f_max);
        noise_from_nps(j,i) = noise^2;
        CFR_30(j,i) = trapz(f,T)/Q_30(j,i);
        X(j,i) =dia(j);
    end
...
ylabel('\fontsize{16}NMR');
xlabel('\fontsize{16}CFR');
title('\fontsize{18}NMR and CFR');

% plot(X,Y,'LineWidth',2);
% legend(leg,'FontSize', 16);
% ylabel('\fontsize{16}NPS [mm^2HU^2]');
% xlabel('\fontsize{16}Spatial Frequency [mm^-^1]');
% title('\fontsize{18}NPS - 36 cm');
% xlim([0 1])
